using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace CWB.App.Models.WorkOrder
{
    public class TempSubCon_ListVM
    {
        public long TempSubCon_ListId { get; set; }
        public long ActiveId { get; set; }
        public long Wo_Id { get; set; }
        public long Opr_No { get; set; }
        public long Supplier_Id { get; set; }
        public long Mode { get; set; }
        public long Act_Qnty { get; set; }
        public long Plan_Qnty { get; set; }
        public long IssueQnty { get; set; }
        public long QntyAvl { get; set; }
        public long Bal_Qnty { get; set; }
        public char Loaded { get; set; }
        public char Rework_Wo { get; set; }
        public DateTime Plan_Disp_date { get; set; }
        public DateTime Plan_Recpt_date { get; set; }
        public DateTime Act_Disp_date { get; set; }
        public DateTime Act_Recpt_date { get; set; }
        public long TenantId { get; set; }
        public int QntySent { get; set; }
        public int QntyRecd { get; set; }
        public int BalToReceive { get; set; }
        public string DataChanged { get; set; }
        public string PartNo { get; set; }
        public string RoutingName { get; set; }
        public string OprNoName { get; set; }
        public string PsEndDate { get; set; }
        public string PsStartDate { get; set; }
        public string CsEndDate { get; set; }
        public string CsStartDate { get; set; }
        public string WoNumber { get; set; }
        public int PartCount { get; set; }
        public int Changed { get; set; }
        public int PartMatlSent { get; set; }
        public int PartToSent { get; set; }
        public string DateNotLoaded { get; set; }
        public string Supplier { get; set; }
    }
}
